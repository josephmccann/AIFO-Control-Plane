...
_SCHEMA_VERSION = 1
_PATH_ERRORS = (OSError, ValueError, UnicodeError, OverflowError, sqlite3.Error)
...
def consume_once(store_path: str, bindings: Sequence[ConsumptionBinding]) -> bool:
    ...
    if (
        type(store_path) is not str or not store_path or store_path == ":memory:"
        or "\x00" in store_path
        or any(0xD800 <= ord(character) <= 0xDFFF for character in store_path)
        or store_path.startswith("file:")
        or not isinstance(bindings, (list, tuple)) or not bindings
    ):
        return False
    try:
        os.fsencode(store_path)
    except _PATH_ERRORS:
        return False
    values = []
