...
class CommandTests(unittest.TestCase):
    ...
    def test_activation_consumption_proposal_uses_validator_terminal_result(self):
        ...
        mission, policy = valid_context()
        ready_source = source_comment(26, "agent-a", "/eos ready")
        ready = audit_event(
            "mission.ready", "agent-a", "producer", ready_source["html_url"],
        ...
        auth_source = source_comment(27, "josephmccann", "/eos authorize-baseline nonce-012345678901234567890123456789")
        ...
        run = {"id": 27, "html_url": run_url, "name": "Mission command", "path": ".github/workflows/mission-command.yml",
               "event": "issue_comment", "repository": {"full_name": REPOSITORY}}
        auth = audit_event("test_integrity.baseline.authorized", "josephmccann", "founder", auth_source["html_url"],
                           auth_source["created_at"], 2, ready["event_hash"], activation)
        attempt_source = source_comment(28, "josephmccann", "/eos attempt-baseline nonce-012345678901234567890123456789")
        attempt = authorize_command_proposal(
            [ready_source, event_comment(29, [ready]), auth_source, event_comment(30, [auth]), attempt_source],
        ...
        consume_source = source_comment(31, "josephmccann", "/eos consume-baseline nonce-012345678901234567890123456789")
        consume = authorize_command_proposal(
            [ready_source, event_comment(29, [ready]), auth_source, event_comment(30, [auth]), attempt_source,
             event_comment(32, [attempt.events[0]]) , consume_source],
