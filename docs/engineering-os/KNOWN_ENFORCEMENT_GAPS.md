...
| GAP-PRIVATE-ACTIONS-ACCESS | Platform limitation | High: personal-account sharing exposes private Actions components to every private repository owned by the account | Founder-approved `access_level=user`; exact repository, owner, caller workflow, event/ref, called workflow, and SHA gates; pinned private-action kernel transport | Repository owner | Revert access to `none` if any caller boundary or private-action provenance check fails |
