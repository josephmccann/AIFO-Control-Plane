...
Status: Documentation-only plan based on `AI.FO-Demo` `master` at `3329c99beb0713269b54bc5fd6a7fb39bf44f398`. No product code, AWS resource, secret, callback, DNS, or customer data is changed by this plan.
...
1 controlled migrations
  ...
  -> 4 session/CSRF
  -> 5 upload quarantine
  -> 6 structured redaction
  -> 7 customer deletion
  -> 8 AI/verifier policy
  -> 9 readiness/health
  -> 10 reproducible artifact
```

Items 5, 6, and 8 may be developed in parallel after item 4, but item 7 consumes their storage/log/provider inventories, item 9 consumes all dependency checks, and item 10 is the final deployable-artifact proof. No customer data may enter staging; production remains blocked until every production gate passes.

## 1. Controlled Schema Migrations
...
**PR #186 overlap:** Partial. Migration `0011` is explicit and additive, and connector writes are transactional/idempotent. It does not establish a single ledger, remove startup DDL, add foreign keys/RLS/checks to `external_connections` or `source_observations`, or define expand/contract and rollback.

**Implementation:** Select one migration runner with an immutable ledger; baseline existing schemas; make startup schema mutation fatal/absent; add preflight schema-version checks; require expand/migrate/contract ordering; add constraints only after orphan/invalid-row checks. Treat connector tables as first-class migration and restore scope.

**Acceptance/tests:** Empty-database migration; upgrade from a production-like schema fixture; repeat run is a no-op; concurrent runner lock; application refuses incompatible schema; rollback rehearsal restores the pre-migration snapshot or forward-fixes; connector uniqueness and transaction tests remain green.

**Migration/rollback:** Back up first, run a dry-run inventory, deploy additive schema before readers/writers, backfill with resumable batches, validate counts/constraints, then contract in a later release. Roll back application while schema remains backward-compatible; restore/PITR only under the approved recovery procedure.
...
**Blocking:** Blocks staging with QBO secrets, production, and all customer data. Synthetic staging without QBO credentials may proceed only after item 1.

**ADR:** No new ADR. ADR-0015 already owns the keyring and rotation decision.

## 3. Database-Backed Tenant-Isolation Testing
...
## 5. Upload Quarantine, Integrity, And Deterministic Ordering
...
## 8. AI-Provider Abstraction And Verifier Fail-Closed Behavior
...
## 9. Readiness And Health Behavior
...
**PR #186 overlap:** Partial tests only. Account readiness includes connector status but masks connector-read failure as an empty list; feature flags can keep connectors out of the critical path. No orchestrator readiness contract was added.

**Implementation:** Keep liveness process-local; add readiness for compatible schema, DB/session store and required object configuration; classify optional QBO/Stripe/AI/verifier dependencies as degraded rather than making every external outage restart tasks; expose no secrets/customer values; drain on SIGTERM; publish bounded dependency metrics.

**Acceptance/tests:** DB/schema/session failure removes readiness; optional provider failure reports degraded and alerts without restart loops; account endpoint distinguishes unavailable from unconfigured; startup and shutdown under load preserve acknowledged work; deployment smoke tests wait for readiness.

**Migration/rollback:** Add endpoint and metrics before changing orchestrator health target; validate in local container/synthetic staging; retain liveness fallback for rollback.

**Blocking:** Blocks staging deployment validation and production. Customer data remains independently blocked by preceding items.

**ADR:** No new ADR. Covered by ADR-0012 and ADR-0021.

## 10. Reproducible Containerized Build And Deployment Artifact

**Merged product files/components affected:** root `package.json`, `pnpm-workspace.yaml`, `pnpm-lock.yaml`; `artifacts/api-server/package.json` (including new Stripe SDK), `build.mjs`, `src/index.ts`; `artifacts/aifo/package.json`, `vite.config.ts`, `prerender.mjs`; `.replit`; `lib/*/package.json`; `db/migrations/*.sql`; readiness from item 9; `artifacts/api-server/src/lib/deployParity.ts`, `src/scripts/deployParity.ts`; `.github/workflows/*`.
