...
| TM-16 | T/D | Failed application deployment or premature health decision causes outage | `3329c99` deployed and smoke passed; prior rollback SHA retained; startup health returned transient 500 then stabilized | Immutable image, separate liveness/readiness, consecutive-success stabilization window, timeout, two tasks, circuit-breaker/alarm rollback, prior digest/prefix, smoke/bake, release runbook | Low | Platform/Product |
| TM-17 | T/D | Automatic or corrupt migration drops data or creates mixed schema | `0011` applied transactionally; a Replit automatic diff later proposed destructive drops and was canceled before promotion; fragmented SQL/Drizzle/startup DDL remain | One ledger, independent dev/prod baselines, exact generated-SQL review, hard stop on destructive/unexplained diff, remove startup DDL, expand/contract, one-off role/task, restore point and forward-fix/PITR test | Medium | Product/Data |
...
6. An automatic development-to-production schema diff proposes destructive drops and is promoted without reviewing the exact SQL.
...
11. Transient startup failures are mistaken for readiness or permanent failure, causing premature traffic routing or unsafe deployment retries.
