...
- **Current activity [FACT]:** Active. Jul 11–14 commits are nightly telemetry/snapshot automation plus telemetry-integrity fixes (#187–#189); last substantive feature wave merged 2026-07-10 (#179–#184: sealed board package, trust evidence console, QBO self-serve recovery, public trust copy).
- **Deployment role [FACT]:** Deploys the live demo at `https://demo.getaifo.com` via Replit autoscale. Also serves `/api/telemetry`, which the marketing site consumes. Nightly pressure test runs from a launchd job on Joe's Mac (02:00 PT) with a GitHub Actions dead-man monitor.
...
- **Open PRs [PR]** (state as of audit — refresh before deciding):
  - **#186** feature: commercial account surface + Stripe/external connectors — the only open code PR. Status: **pending final technical review and founder merge decision**. Its body self-reports 13,487 aifo tests, 1,003 api-server tests, and 6 Chromium E2E green; this verification is **self-reported and has not been independently re-run** — do not assume merge readiness from it.
  ...
  - **#195** session checkpoint docs (memory/, workqueue/, handoff, CHANGELOG). Status: **July 14 shutdown checkpoint that may now require refresh, supersession, or closure** — later activity (PR #13, the live EDGAR collection) has changed the state it froze. Not recommended for automatic merge.
